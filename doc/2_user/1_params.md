...
| `output_folder_path` | `str` | Path of the folder which will contain all generated artifacts | Always | No |
...
| `segment_idx_width` | `int` | Bits indexing segments (for hybrid, bipartite, and symmetric) | When `method_name` in `{"hybrid", "bipartite", "symmetric"}`<br>> Defaults to $\lfloor$`data_width`$/2\rfloor$ | No |
| `group_idx_width` | `int` | Bits indexing group of segments (for bipartite, and symmetric) | When `method_name` in `{"bipartite", "symmetric"}`<br>> Defaults to $\lfloor$`data_width`$/4\rfloor$ | No |
...
| `fpga_working_dir` | `str` | Target FPGA working directory.<br>**WARNING**: Files will be transferred and executed in this folder | When `step` is `impl` | No |
